...
SELECT models.name, models.brand_name, brands.headquarters FROM brands, models WHERE brands.name = models.brand_name AND models.brand_name = 'Ford' AND models.name = 'Mustang';
...
SELECT b.name, b.founded, m.name FROM brands AS b LEFT JOIN models
AS m ON b.name = m.brand_name WHERE b.discontinued is NULL;
...
SELECT b.name, b.founded FROM brands AS b LEFT JOIN models AS m ON b.name = m.brand_name WHERE m.name IS NULL;
...
SELECT models.brand_name FROM models GROUP BY models.brand_name HAVING COUNT(*) > 5;
...
SELECT m.name, m.year FROM models AS m WHERE m.year = (SELECT b.founded FROM brands AS b WHERE b.founded = m.year);